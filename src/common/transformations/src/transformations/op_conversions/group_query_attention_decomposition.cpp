...
ov::pass::GroupQueryAttentionDecomposition::KVCacheOutputs ov::pass::GroupQueryAttentionDecomposition::construct_kvcache(
    ...
    if (node->get_sliding_window_cache()) {
        ...
        if (static_single_token) {
            ...
            outputs.present_key = register_new_node<v3::ScatterUpdate>(zeros, kept_row, survivor_k, scatter_axis);
            outputs.present_key = register_new_node<v3::ScatterUpdate>(outputs.present_key, new_idx, key, scatter_axis);
            outputs.present_value = register_new_node<v3::ScatterUpdate>(zeros, kept_row, survivor_v, scatter_axis);
            outputs.present_value =
                register_new_node<v3::ScatterUpdate>(outputs.present_value, new_idx, value, scatter_axis);

            sdpa_key = outputs.present_key;
            sdpa_value = outputs.present_value;
        ...
        std::shared_ptr<ov::Node> scatter_idx =
            register_new_node<v4::Range>(zero_without_shape, current_seqlen_scalar, one_without_shape, ov::element::i64);
    ...
    outputs.sdpa_key = sdpa_key;
    outputs.sdpa_value = sdpa_value;
    return outputs;
}
